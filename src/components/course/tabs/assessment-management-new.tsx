...
export function AssessmentManagement({ courseId, assessment, onClose, onUpdate }: AssessmentManagementProps) {
  ...
  const [showCOSelectDialog, setShowCOSelectDialog] = useState(false);
  const [selectedQuestionForCO, setSelectedQuestionForCO] = useState<Question | null>(null);
  ...
  const handleManageCOs = (question: Question) => {
    setSelectedQuestionForCO(question);
    setQuestionForm({
      question: question.question,
      maxMarks: question.maxMarks,
      selectedCOs: question.coMappings.map(mapping => mapping.co.id)
    });
    setShowCOSelectDialog(true);
  };
  ...
  const handleUpdateCOs = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`/api/courses/${courseId}/assessments/${assessment.id}/questions/${selectedQuestionForCO.id}/cos`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  return (
    <Dialog open={true} onOpenChange={onClose}>
      <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
                        ...
                        <div className="flex gap-1">
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent>
                              ...
                              <DropdownMenuItem onClick={() => handleManageCOs(question)}>
        ...
        <Dialog open={showQuestionDialog} onOpenChange={setShowQuestionDialog}>
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              ...
              <DialogDescription>
                {editingQuestion ? 'Edit the question details and CO mappings' : 'Create a new question and map it to COs'}
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="border rounded-lg p-3 max-h-40 overflow-y-auto">
                  {cos.length === 0 ? (
                    <p className="text-sm text-gray-500">No COs available. Please create COs first.</p>
                  ) : (
                    <div className="space-y-2">
                      {cos.map((co) => (
                            ...
                            onCheckedChange={(checked) => {
                              if (checked) {
                                setQuestionForm(prev => ({
                                  ...prev,
                                  selectedCOs: [...prev.selectedCOs, co.id]
                                }));
                              } else {
                                setQuestionForm(prev => ({
                                  ...prev,
                                  selectedCOs: prev.selectedCOs.filter(id => id !== co.id)
                                }));
                              }
                            }}
                          />
                          <Label htmlFor={`co-${co.id}`} className="text-sm cursor-pointer">
        ...
        {/* CO Management Dialog */}
        <Dialog open={showCOSelectDialog} onOpenChange={setShowCOSelectDialog}>
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              <DialogTitle>Manage CO Mappings</DialogTitle>
              <DialogDescription>
                Update CO mappings for: {selectedQuestionForCO?.question}
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-4">
              <div className="space-y-2">
                    ...
                    <div className="space-y-2">
                      {cos.map((co) => (
                        <div key={co.id} className="flex items-center space-x-2">
                          ...
                          />
                          <Label htmlFor={`co-select-${co.id}`} className="text-sm cursor-pointer">
                            <span className="font-medium">{co.code}</span>
                            <span className="text-gray-500 ml-2">{co.description}</span>
                          </Label>
                        </div>
              ...
              <div className="flex gap-2 justify-end">
                <Button variant="outline" onClick={() => setShowCOSelectDialog(false)}>
                  Cancel
                </Button>
                <Button onClick={handleUpdateCOs} disabled={loading}>
                  {loading ? 'Updating...' : 'Update CO Mappings'}
                </Button>
              </div>
            </div>
          </DialogContent>
